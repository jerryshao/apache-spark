...
class RateStreamContinuousReader(options: DataSourceOptions)
  ...
  val numPartitions = options.get(RateSourceProvider.NUM_PARTITIONS).orElse("5").toInt
  val rowsPerSecond = options.get(RateSourceProvider.ROWS_PER_SECOND).orElse("6").toLong
